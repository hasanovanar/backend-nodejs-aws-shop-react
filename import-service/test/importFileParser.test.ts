...
// Mock csv-parser
jest.mock("csv-parser", () => jest.fn(() => Readable.from(["name,age\nJohn,30\nDoe,40"])));
...
describe("importFileParser Lambda Function", () => {
  ...
  it("should process the CSV file and move it to the parsed folder", async () => {
    const bodyStream = Readable.from(["name,age\nJohn,30\nDoe,40"]);
...
describe("importFileParser Lambda Function", () => {
  ...
  it("should process the CSV file and move it to the parsed folder", async () => {
    ...
    // Verify CopyObjectCommand was called
    expect(s3Mock.calls(CopyObjectCommand)).toHaveLength(1);
    expect(
    ...
    // Verify DeleteObjectCommand was called
    expect(s3Mock.calls(DeleteObjectCommand)).toHaveLength(1);
    expect(
